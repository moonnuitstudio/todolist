...
const InputBase = ({ id, title, placeholder=null, type="text", values=[], disabled=false, endIcon=null, defaultvalue="", minDate=undefined, onEndIconClick=null }:InputBasePropsType) => {
    ...
    const [fieldValue, setFieldValue] = React.useState<number | string | Date>(defaultvalue)
    ...
    const InputElement = React.useMemo(():React.ReactNode => {
        switch(type) {
            case "time":
                return (<InputTime value={(fieldValue as string)} disabled={disabled} id={id} placeholder={placeholder || ""} setValue={setValue} />)
            case "date":
                return (<InputDate value={(fieldValue)} disabled={disabled} id={id} placeholder={placeholder || ""} setValue={setValue} minDate={minDate} />)
            case "textarea":
            ...
            case "select": 
                return (<InputSelect disabled={disabled} id={id} values={values} setValue={setValue} value={fieldValue} />)
            default:
                return (<input id={`input-${id}`} className={inputClassNames} type={type} placeholder={placeholder || ""} {...register(id)} />)
        }
    }, [type, values, id, inputClassNames, register, disabled, placeholder, fieldValue, minDate, setValue])

    return (
        <InputContainer>
            <label htmlFor={`input-${id}`} className={styles.fieldName}>{title}</label>
            <InputBox borderColor={inputBorderColor} sx={{ ...(!disabled && {'&:hover': { boxShadow: `0px 5px 20px -10px ${inputBorderColor}` }}) }}>
                {InputElement}
